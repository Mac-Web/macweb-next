...
async function Profile() {
  ...
  await dbConnect();
  const existingUser = await User.findOne({ username: session?.user?.email ? session.user.email : session?.user?.name });

  return (
    <>
      {existingUser ? (
        <div className="flex flex-col gap-y-7 rounded-lg bg-gray-900 w-85 p-10">
          {existingUser.image ? (
            <Image
